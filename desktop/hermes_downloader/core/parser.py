...
class VideoInfo:
    ...
    duration: str
    thumbnail: str | None
    formats: list[str]       # доступные разрешения
    filesize: str | None     # примерный размер
    uploader: str | None
    url: str


def parse_video(url: str, proxy: str = None) -> VideoInfo | None:
    """
    Извлекает информацию о видео без загрузки.

    Возвращает VideoInfo или None, если видео недоступно.
    """
    ydl_opts = {
        'quiet': True,
        'no_warnings': True,
